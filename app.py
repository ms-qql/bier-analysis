...
if app_mode == "Feature Analysis":
        ...
        if st.button("Run Lag Analysis (Stage 5)", type="primary"):
             if 'fa_results' not in st.session_state or st.session_state.fa_results is None:
                 ...
                 if target_col not in df_full.columns:
                     ...
                     # Save Significant Lags to Session State for Stage 8
                     all_sig_lags = sorted(list(set(sig_acf + sig_pacf)))
                     st.session_state['stage5_lags'] = all_sig_lags
                     st.write(f"**Lags saved for Stage 8**: {all_sig_lags}")
    ...
    with st.expander("Stage 6: Market Regime Detection", expanded=True):
        ...
        regime_method = st.selectbox("Detection Method", ["Technical (SMA Trend)", "Machine Learning (GMM Clustering)", "Probabilistic (HMM)"])
             ...
             if 'fa_results' not in st.session_state or st.session_state.fa_results is None:
                 ...
                 if target_col not in df_full.columns:
                     ...
                     if "HMM" in regime_method:
                         method_code = "hmm"
                     elif "Technical" in regime_method:
                         method_code = "technical" 
                     else:
                         method_code = "ml"
                     ...
                     with st.spinner(f"Detecting Regimes using {regime_method}..."):
                         try:
                             ...
                             colors = {
                                 'Bull': 'rgba(0, 255, 0, 0.4)',      # Stronger Green
                                 'Bear': 'rgba(255, 0, 0, 0.4)',      # Stronger Red
                                 'Neutral': 'rgba(200, 200, 200, 0.3)' # Visible Gray
                             }
                             
                             # Plot dummy traces for Legend
                             for r_name, r_color in colors.items():
                                 fig_regime.add_trace(go.Scatter(
                                     x=[None], y=[None],
                                     mode='markers',
                                     marker=dict(size=10, color=r_color),
                                     name=f"{r_name} Regime"
                                 ))
                             
                             for idx, row in blocks.iterrows():
                                 regime = idx[0]
                                 start_date = row['<lambda_0>']
                                 end_date = row['<lambda_1>']
                                 
                                 # Handle single day blocks (add 12h padding)
                             ...
                             c3.metric("Neutral Days", counts['Neutral'])
                             
                             st.success("Regime Detection Complete. You can now proceed to Stage 7.")
                             
                             # Store df_regime in session state to pass to Stage 7
                             st.session_state.df_regime_results = df_regime
                             
                         except Exception as e:
                             st.error(f"Error in Regime Detection: {e}")

    # Stage 7 Expander
    with st.expander("Stage 7: Regime-Specific Feature Importance", expanded=True):
        st.markdown("### Stage 7: Feature Ranking by Regime")
        st.info("Compare how features perform differently in Bull vs. Bear markets. Boruta is run on each subset independently.")
        
        if st.button("Run Regime-Specific Analysis (Stage 7)", type="primary"):
            if 'df_regime_results' not in st.session_state or st.session_state.df_regime_results is None:
                st.error("Please run Stage 6 (Regime Detection) first to classify the data.")
            else:
                 df_regime_input = st.session_state.df_regime_results
                 
                 # Merge back other features? 
                 # df_regime_results only has date, close, Regime. We need the full features.
                 # Re-fetch full data and merge Regime.
                 start_str_fa = start_date_fa.strftime('%Y-%m-%d')
                 end_str_fa = end_date_fa.strftime('%Y-%m-%d')
                 df_full, _ = matrix_strategy.get_strategy_df(start_str_fa, end_str_fa, asset="btc")
                 
                 # Merge Regime column on Date
                 df_full['date'] = pd.to_datetime(df_full['date'])
                 df_regime_input['date'] = pd.to_datetime(df_regime_input.index) # Index was date
                 
                 # Drop duplicate regime input date col if needed or just use index join
                 # df_regime_input has date as index and column due to my previous code? 
                 # Let's align carefully.
                 
                 # Create mapping
                 regime_map = df_regime_input['Regime'].to_dict() # Map Date -> Regime
                 df_full['Regime'] = df_full['date'].map(regime_map)
                 
                 # FILTER: Use only 'test' category features + required cols
                 df_categories = database.read_categories()
                 test_metrics, _ = database.load_category_list('test', metric='', df=df_categories)
                 
                 # Keep columns: date, close, Regime + test_metrics
                 # (Ensure we don't duplicate if close is in test_metrics)
                 cols_to_use = ['date', 'close', 'Regime'] + [c for c in test_metrics if c in df_full.columns and c not in ['date', 'close', 'Regime']]
                 df_stage7 = df_full[cols_to_use].copy()
                 
                 st.write(f"Analyzing {len(cols_to_use)-3} features (Category 'test').")
                 
                 with st.spinner("Running Boruta on Global, Bull, Bear, and Neutral subsets (this may take a minute)..."):
                     comp_df = feature_analysis.analyze_regime_specific_importance(df_stage7)
                     
                     st.write("### Feature Rank Comparison (Lower is Better)")
                     st.caption("Rank 1 = Top Feature. Rank 99 = Rejected.")
                     
                     # Heatmap styling: Low ranks (green), High ranks (red)
                     st.dataframe(comp_df.style.background_gradient(cmap='RdYlGn_r', axis=None), use_container_width=True)
                     
                     st.download_button(
                        label="Download Regime Rankings (CSV)",
                        data=comp_df.to_csv(),
                        file_name="bier_regime_feature_ranks.csv",
                        mime="text/csv",
                        key="download_regime_ranks"
                     )
                     
                     st.success("Comparison Complete. Rankings saved for Stage 8.")
                     st.session_state['stage7_ranks'] = comp_df
                     
                     
    # Stage 8 Expander
    with st.expander("Stage 8: XGBoost Validation (Regime Enhanced)", expanded=True):
        st.markdown("### Stage 8: Regime-Switching Model Benchmark")
        st.info("Trains specialized models for Bull, Bear, and Neutral markets using insights from Stage 5 (Lags) and Stage 7 (Features).")
        
        xgb_lookahead = st.slider("Target Lookahead (Days)", 1, 30, 5, key="xgb_lookahead")
        
        # Check inputs
        has_lags = 'stage5_lags' in st.session_state
        has_ranks = 'stage7_ranks' in st.session_state
        
        if not has_lags:
            st.warning("⚠️ Stage 5 (Lags) not run yet. Momentum features will not be optimized.")
        if not has_ranks:
            st.warning("⚠️ Stage 7 (Regime Ranks) not run yet. Feature selection will use all features (slower/noisier).")
            
        if st.button("Train Regime-Switching Model (Stage 8)", type="primary"):
            if 'fa_results' not in st.session_state or st.session_state.fa_results is None:
                st.error("Please run the Correlation Analysis (Stage 1) first to load the data.")
            else:
                 start_str_fa = start_date_fa.strftime('%Y-%m-%d')
                 ...
                 # Prepare Regime Column (Re-run detection if needed or use session state)
                 if 'df_regime_results' in st.session_state:
                     df_regime_input = st.session_state.df_regime_results
                     df_full['date'] = pd.to_datetime(df_full['date'])
                     df_regime_input['date'] = pd.to_datetime(df_regime_input.index)
                     regime_map = df_regime_input['Regime'].to_dict()
                     df_full['Regime'] = df_full['date'].map(regime_map)
                 else:
                     st.error("Stage 6 (Regime Detection) results missing. Please run Stage 6.")
                     st.stop()
                 
                 # FILTER: Use only 'test' category features
                 df_categories = database.read_categories()
                 test_metrics, _ = database.load_category_list('test', metric='', df=df_categories)
                 base_features = [c for c in test_metrics if c in df_full.columns and c not in ['date', 'close', 'Regime']]
                 
                 # Inputs from prev stages
                 sig_lags = st.session_state.get('stage5_lags', [])
                 regime_ranks = st.session_state.get('stage7_ranks', None)
                 
                 st.write(f"Training Specialist Models on {len(base_features)} base features + {len(sig_lags)} momentum features...")
                 
                 with st.spinner("Training Specialist Models (Bull/Bear/Neutral)..."):
                     metrics, importances, results_df = feature_analysis.train_regime_switching_xgboost(
                         df_full, 
                         base_feature_cols=base_features, 
                         regime_ranks=regime_ranks,
                         significant_lags=sig_lags,
                         target_col='close', 
                         lookahead=xgb_lookahead
                     )
                     
                     if metrics is None:
                         st.error("Training failed. Likely not enough data.")
                     else:
                         # 1. Metrics
                         c1, c2, c3, c4 = st.columns(4)
                         c1.metric("RMSE", f"{metrics['RMSE']:.4f}")
                         c2.metric("R2 Score", f"{metrics['R2']:.4f}")
                         c3.metric("Directional Accuracy", f"{metrics['Directional Accuracy']*100:.1f}%")
                         c4.metric("Test Coverage", f"{metrics['Test Coverage']*100:.1f}%")
                         
                         if metrics['Directional Accuracy'] > 0.53:
                             st.success(f"Strong Accuracy: {metrics['Directional Accuracy']*100:.1f}%")
                         else:
                             st.warning(f"Accuracy: {metrics['Directional Accuracy']*100:.1f}%")
                         
                         st.divider()
                         
                         # 2. Importance Tabs
                         st.subheader("Regime-Specific Drivers")
                         tab1, tab2, tab3 = st.tabs(["Bull Drivers", "Bear Drivers", "Neutral Drivers"])
                         
                         with tab1:
                             if 'Bull' in importances:
                                 fig_bull = px.bar(importances['Bull'].head(10), x='Importance', y='Feature', orientation='h', title="Top Bull Features")
                                 fig_bull.update_layout(yaxis={'categoryorder':'total ascending'})
                                 st.plotly_chart(fig_bull, use_container_width=True)
                             else:
                                 st.write("No Bull Model trained.")
                         
                         with tab2:
                             if 'Bear' in importances:
                                 fig_bear = px.bar(importances['Bear'].head(10), x='Importance', y='Feature', orientation='h', title="Top Bear Features")
                                 fig_bear.update_layout(yaxis={'categoryorder':'total ascending'})
                                 st.plotly_chart(fig_bear, use_container_width=True)
                             else:
                                 st.write("No Bear Model trained (insufficient data?).")
                                 
                         with tab3:
                             if 'Neutral' in importances:
                                 fig_neut = px.bar(importances['Neutral'].head(10), x='Importance', y='Feature', orientation='h', title="Top Neutral Features")
                                 fig_neut.update_layout(yaxis={'categoryorder':'total ascending'})
                                 st.plotly_chart(fig_neut, use_container_width=True)
                        
                         # 3. Equity Curve
                         st.subheader("Strategy Simulation (Dynamic Switching)")
                         results_df['Cum_Strategy'] = (1 + results_df['Strategy_Returns']).cumprod()
                         results_df['Cum_BuyHold'] = (1 + results_df['Actual']).cumprod()
                         
                         fig_eq = go.Figure()
                         fig_eq.add_trace(go.Scatter(x=results_df['date'], y=results_df['Cum_Strategy'], name='Switching Strat', line=dict(color='purple', width=2)))
                         fig_eq.add_trace(go.Scatter(x=results_df['date'], y=results_df['Cum_BuyHold'], name='Buy & Hold', line=dict(color='gray', dash='dash')))
                         
                         # Color background by regime in prediction chart?
                         # (Optional: Add later if needed, simple curve is enough for now)
                         
                         st.plotly_chart(fig_eq, use_container_width=True)
                         
                         st.success("Regime-Switching Validation Complete.")
